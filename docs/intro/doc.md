...
* ![options](imgs/options.png)选项按钮：点击选项按钮，会弹开如下的对话框，在这个弹出框中可以进行CPU的初始化设置：
    ...
    * “Memory values”编辑框中可以设置内存的初始值，格式是每行两个数字描述一个内存的值，每行的前一个数值表示内存地址，第二个值表示该内存中的值，内存地址为0~4095
...
<p style="text-align: center"><img src="imgs/options_dialog.png"/></p>

### 状态机

上面5个按钮的各个功能可以用以下状态机进行描述：

<p style="text-align: center"><img src="imgs/status.png"/></p>

### 指令说明

目前本模拟器仅支持以下6条指令，通过选项按钮打开的指令编辑框中每行指令必须是以下6条格式的指令之一：

1. `ADDD F1, F2, F3`：加法指令，计算`F1=F2+F3`,其中`F1`,`F2`,`F3`均为浮点寄存器的名字
2. `SUBD F1, F2, F3`：减法指令，计算`F1=F2-F3`,其中`F1`,`F2`,`F3`均为浮点寄存器的名字
3. `MULD F1, F2, F3`：乘法指令，计算`F1=F2*F3`,其中`F1`,`F2`,`F3`均为浮点寄存器的名字
4. `DIVD F1, F2, F3`：除法指令，计算`F1=F2/F3`,其中`F1`,`F2`,`F3`均为浮点寄存器的名字
5. `LD F1, ADDR`：载入指令，将内存中地址为`ADDR`的值载入浮点寄存器`F1`,其中`F1`为浮点寄存器的名字，`ADDR`为地址，格式为`32(R1)`,其中`R1`是整数寄存器的名字，`32`是地址相对`R1`的偏移值
6. `ST F1, ADDR`：储存指令，将浮点寄存器`F1`的值写入内存中地址为`ADDR`的内存，其中`F1`为浮点寄存器的名字，`ADDR`为地址，格式为`24(R4)`,其中`R4`是整数寄存器的名字，`24`是地址相对`R4`的偏移值

**注：内存地址为0~4095，请不要试图访问或写入不在此范围的内存。**
...
“Memory”表格在软件的右上方，描述当前内存的状况，**由于内存一般来说非常大，所以只有被初始化和被修改了的内存会被显示出来**；“Memory”表格每行描述一块内存，一共两列，其含义如下：
